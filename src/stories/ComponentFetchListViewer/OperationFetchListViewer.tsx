...
const OperationFetchListViewer: FC = () => {
    const [items, setItems] = useState(Array.from({ length: 20 }).map(() => (createRandomOperation(getRandomDate(new Date('2022-01-01'), new Date('2022-12-31'))))))

    const doFetch = () => {
        setItems([
            ...items,
        ...(Array.from({ length: 20 }).map(() => (createRandomOperation(getRandomDate(new Date('2022-01-01'), new Date('2022-12-31'))))))
        ])
    }
